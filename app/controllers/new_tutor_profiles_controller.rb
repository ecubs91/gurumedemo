class NewTutorProfilesController < Wicked::WizardController
    ...
    if params[:id]=="finish"
      ...
      if params[:id]=="step1"
        if (@tutor_profile.country.present? rescue false)
          @cities = Country.find_by(:name => @tutor_profile.country).cities.pluck(:name)
        else
          @cities = []
        end
  ...
  private
  def step_1_params
    params.require(:tutor_profile).permit(:university, :hourly_rate, :location, :location_two, :contact_num, :location_two, :country, :detailed_location, :contact_details)
  end
